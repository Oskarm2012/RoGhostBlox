...
function placeObstacle(board, obstacle, startX, startY){
    for(coordinadePair of obstacle){
        [x,y] = coordinadePair;
...
function endGame(){
    alert('<Game over>Pelisi on pelattu "sinäkin brutukseni?" <Game over>')
...
class Player{
    ...
    move(deltaX, deltaY){
        //otetaan talteen tämän hetken sijainti
...
class Ghost{
    ...
    moveGhostTowardsPlayer(player, board, oldGhosts){
        ...
        if(Math.abs(dx) > Math.abs(dy)){
            ...
            else moves.push({x: this.x, y: this.y - 1}) //ylöspäin
        }else{
             //pystyssä liikkuminen
            if(dy > 0) moves.push({x: this.x, y: this.y + 1}) //liikutaan alaspäin
            else moves.push({x: this.x, y: this.y - 1}) //ylöspäin
